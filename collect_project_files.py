...
# ============================================================================
# 1. Сохраните этот файл как collect_project_files.py в папке:
...
#        python collect_project_files.py
#
#    Способ C (простой, если Python установлен):
#      - Откройте папку D:\Android\Projects\SpeleoStartApp
#      - Дважды кликните на collect_project_files.py
#        (НО: окно может быстро закрыться, лучше использовать способы A или B)
#
#    Способ D (через PyCharm/Android Studio):
#      - Откройте файл в IDE
#      - Нажмите правой кнопкой -> Run 'collect_project_files'
#
...
def check_python_version():
    ...
    if sys.version_info < (3, 6):
        ...
        print("   Скрипт может работать, но лучше обновиться")

def show_help():
    """Показывает быструю справку по запуску"""
    print("\n" + "=" * 70)
    print("❓ КАК ЗАПУСТИТЬ СКРИПТ ЕЩЁ РАЗ:")
    print("=" * 70)
    print("1. Откройте командную строку (Win+R → cmd)")
    print("2. Введите команды:")
    print(f"   cd {PROJECT_DIR}")
    print("   python collect_project_files.py")
...
def main():
    ...
    # Быстрая справка по запуску (только при первом запуске)
    if not (PROJECT_DIR / f"{OUTPUT_BASE_NAME}_1.txt").exists():
        show_help()

    print("🔍 Поиск файлов в проекте...")
    if not PROJECT_DIR.exists():
        print(f"❌ Ошибка: папка проекта не найдена: {PROJECT_DIR}")
        print(f"\n💡 Совет: проверьте путь в переменной PROJECT_DIR в начале скрипта")
        sys.exit(1)
    ...
    for idx, (rel_path, full_path) in enumerate(files, 1):
        ...
        try:
            if full_path.suffix in {".png", ".jpg", ".jpeg", ".zip", ".pdf", ".svg"}:
    ...
    print("\n📝 Чтобы запустить скрипт ещё раз:")
